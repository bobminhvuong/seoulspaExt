...
chrome.extension.onMessage.addListener(function(request, sender, sendResponse) {
    ...
    if (request.type == 'sendAndClear') actionToSendAndClear(request);
...
function getNoteDetail(request) {
    chrome.storage.sync.get(['phone'], function(result) {
        ...
        fetch(base_url + '/api/index.php/pancake/GetCustomerNote', {
            method: 'post',
...
function login(phone, pass) {
    ...
    if (phone == '000' && pass == '123') {
        ...
        fetch(base_url + '/api/index.php/pancake/login', {
            method: 'post',
            body: JSON.stringify(arrSend)
        }).then(r => {
            r.json().then(function(data) {
                data.type = 'login';
                sendAndClearData('user');
                sendMessageToContent(data);
                getGroupService_bg(data.user.phone);
            })
            chrome.storage.sync.set({ 'dateLogin': (new Date()).toISOString() }, function() {});
        })
    }
}

function actionToSendAndClear(request) {
    sendAndClearData(request.phone);
    // let data = {
    //     message: 'Gửi thông tin thành công!',
    //     status: 200,
    //     type: 'autoSendAndClear',
    // }
    // sendMessageToContent(data);
}

function sendAndClearData(phone) {
    chrome.storage.sync.get(['inboxUserData'], function(result) {
        if (result.inboxUserData) {
            result.inboxUserData.forEach(element => {
                element.api_key = '1DB185DCFB40836B29BFC1A500E3EB';
